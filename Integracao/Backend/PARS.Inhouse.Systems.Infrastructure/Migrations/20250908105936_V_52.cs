...
{
    /// <inheritdoc />
    public partial class V_52 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Rename apenas se a coluna existir
            migrationBuilder.Sql(@"
                IF EXISTS (
                    SELECT 1
                    FROM sys.columns 
                    WHERE Name = N'Fabricante'
                      AND Object_ID = Object_ID(N'IntegracaoSppBimerInvoce')
                )
                BEGIN
                    EXEC sp_rename 
                        'IntegracaoSppBimerInvoce.Fabricante', 
                        'FabricanteId', 
                        'COLUMN';
                END
            ");
            ...
            migrationBuilder.CreateTable(
                ...
                constraints: table =>
                {
                    table.PrimaryKey("PK_IntegracaoBacenLogErros", x => x.Id);
                });
        }
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            // Volta o nome apenas se FabricanteId existir
            migrationBuilder.Sql(@"
                IF EXISTS (
                    SELECT 1
                    FROM sys.columns 
                    WHERE Name = N'FabricanteId'
                      AND Object_ID = Object_ID(N'IntegracaoSppBimerInvoce')
                )
                BEGIN
                    EXEC sp_rename 
                        'IntegracaoSppBimerInvoce.FabricanteId', 
                        'Fabricante', 
                        'COLUMN';
                END
            ");
